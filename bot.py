...
NOTIFY_ORDER = ["new_files", "exam_chat", "casual_chat", "startup", "schedule_change"]
...
def save_class_exceptions(data: dict) -> None:
    with open(CLASS_EXCEPTIONS_FILE, "w", encoding="utf-8") as f:
...
def get_class_exception_dates(program: str, day: str, entry: dict) -> list:
    ...
    return list(exceptions.get(program, {}).get(day, {}).get(_class_entry_key(entry), []))
...
def load_fixed_events() -> list:
    """رویدادهای ثابتِ هفتگیِ بدونِ‌درس (فاز ۱۶) -- مثلِ «دوشنبه فرهنگی»:
    [{name, day, start, end, lead_minutes, scope}] -- scope یا 'all' (برای
    همه‌ی ترم‌ها) یا اسمِ دقیقِ یه ترمِ خاص. lead_minutes پیش‌فرضش ۱۵ه، ولی هر
    رویداد می‌تونه زمانِ یادآوریِ خودشو داشته باشه (نگاه کن به check_reminders)."""
...
def save_fixed_events(events: list) -> None:
    with open(FIXED_EVENTS_FILE, "w", encoding="utf-8") as f:
...
def _parse_time_range(time_part: str):
    """'HH:MM-HH:MM' -> (start, end) یا None اگه فرمت/مقدار نامعتبر بود."""
    if "-" not in time_part:
        return None
...
def filter_class_entries_for_student(entries: list, user_id: int):
    ...
    ۱۳ -- گروه‌بندی): بلوک‌های بدونِ group_id (یعنی برای همه) همیشه می‌مونن؛
...
async def editcourse_pick_course(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not can_manage_term(update.effective_user.id, course["term_name"]):
        ...
        return
    await query.edit_message_text(
        *_course_manage_text_markup(course_id, course),
    )
...
async def course_set_active(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await query.edit_message_text(*_course_manage_text_markup(course_id, course))
...
def _course_groups_text_markup(course_id: int, course: dict):
    ...
    buttons = []
    if not has_split:
        lines.append("\nاین درس الان گروه‌بندی نداره.")
        buttons.append(
            [InlineKeyboardButton("✅ فعال‌کردنِ گروه‌بندی", callback_data=f"crs_groups_toggle:{course_id}")]
        )
    else:
        if groups:
            lines.append("")
...
async def receive_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if pending_class:
        ...
        for line_no, raw in enumerate(raw_lines, start=1):
            ...
            # تشخیصِ تداخل: گروه‌آگاه -- دو بلوکِ هم‌زمان فقط وقتی واقعاً تداخل
            # حساب می‌شن که مخاطبِ مشترک داشته باشن (هردو بی‌گروه، یا دقیقاً
            # همون یه گروه). دو بلوکِ هم‌زمانِ گروه‌های متفاوت (مثلِ پسر/دختر)
            # تداخل نیستن.
            conflict = None
            for existing_entry in batch_for_conflict:
                if not _ranges_overlap(start, end, existing_entry["start"], existing_entry["end"]):
                    continue
                other_group = existing_entry.get("group_id")
                if group_id is None or other_group is None or group_id == other_group:
            ...
            new_entry = {"start": start, "end": end, "course": course_part}
            if course_id is not None:
    ...
    if pending_group:
        ...
        if day_part not in DAYS_OF_WEEK:
            ...
            return
        if "-" not in time_part:
            await update.message.reply_text("فرمتِ ساعت درست نیست. مثال: 10:00-12:00")
            return
        start, end = [p.strip() for p in time_part.split("-", 1)]
        if not (valid_time(start) and valid_time(end)):
            await update.message.reply_text("ساعت باید به فرمت HH:MM باشه. مثال: 10:00-12:00")
            return
    ...
    if pending_exc:
        ...
        program, day, idx = pending_exc["program"], pending_exc["day"], pending_exc["idx"]
        if not can_manage_term(update.effective_user.id, program):
            await update.message.reply_text(f"⛔️ شما دسترسی مدیریت «{program}» رو ندارید.")
    ...
    if pending_fe:
        ...
        if len(parts) < 3 or not parts[0]:
            await update.message.reply_text(
                "فرمت: نام | روز | ساعتِ‌شروع-ساعتِ‌پایان | دقیقه‌ی یادآوریِ‌قبلش(اختیاری)\n"
                "مثال: دوشنبه فرهنگی | دوشنبه | 10:00-12:00 | 60"
            )
            return
        ...
        events.append(
            {"name": name, "day": day, "start": start, "end": end, "lead_minutes": lead_minutes, "scope": scope}
        )
        save_fixed_events(events)
        dbmod.log_action(
            update.effective_user.id, "ADD_FIXED_EVENT",
            detail=f"{name} {day} {start}-{end} scope={scope} lead={lead_minutes}",
        )
        context.user_data.pop("pending_fixed_event", None)
...
async def render_today_agenda(query, user_id: int, program: str) -> None:
    ...
    for c in classes:
        ...
        items.append((c["start"], f"🏫 {c['start']}–{c['end']} | {c['course']}{loc}"))
...
async def today_agenda(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not program:
        buttons = [
            [InlineKeyboardButton(p, callback_data=f"pick_class_program_agenda:{p}")] for p in get_program_names()
        ]
...
async def show_class_day(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not classes and not missing_groups:
        ...
        for c in classes:
            loc = f" — {c['location']}" if c.get("location") else ""
            lines.append(f"⏰ {c['start']}–{c['end']} | {c['course']}{loc}")
        if missing_groups:
            lines.append("")
...
async def render_cls_day(query, program: str, day: str) -> None:
    ...
    for i, c in enumerate(classes):
        ...
        exc_data = f"cls_exc:{term_id}:{day}:{i}"
        ...
        if validate_callback_data(exc_data, context_label=f"cls_exc/program={program}/day={day}/i={i}"):
            buttons.append(
                [InlineKeyboardButton(f"🚫 لغوِ یه‌روزِ «{c['course']}»", callback_data=exc_data)]
            )
...
async def cls_add_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await query.edit_message_text(
        f"باشه، کلاس(های) جدید برای «{day}» رو به این فرم بفرست — هر کلاس تو یه خط، "
        "می‌تونی چندتا خط با هم بفرستی:\n\n"
        "ساعت‌شروع-ساعت‌پایان | نام‌درس | گروه(اختیاری) | مکان(اختیاری)\n\n"
        "مثال‌ها:\n"
        "08:00-10:00 | پاتولوژی\n"
        "10:00-12:00 | باکتری‌شناسی | گروه ۱\n"
        "13:00-15:00 | انقلاب | پسر | سالن آمفی‌تئاتر\n\n"
        "(اسمِ درس رو دقیقاً مثلِ اسمی که توی «📚 دروس» ثبت شده بنویس تا دکمه‌ی ورود به منابع کار کنه؛ "
        "اسمِ گروه هم باید دقیقاً همونی باشه که قبلاً از «مدیریتِ گروه‌بندی» تعریف کردی)",
        reply_markup=back_btn,
    )
...
async def cls_delete(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    _, term_id_str, day, idx_str = query.data.split(":", 3)
    try:
        term_id = int(term_id_str)
    except ValueError:
        term_id = None
    program = program_for_term_id(term_id)
    if program is None:
        logger.error("cls_delete: no term found for term_id=%r", term_id_str)
...
async def cls_exception_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    entry = classes[idx]
    clear_admin_flow_state(context)
    context.user_data["pending_class_exception"] = {"program": program, "day": day, "idx": idx}
    ...
    await query.edit_message_text(
        f"تاریخی که می‌خوای «{entry['course']}» ({day} "
        f"{entry['start']}-{entry['end']}) توش لغو بشه رو بفرست -- شمسی یا میلادی، فرقی نداره "
        "(مثلاً 1405/07/15 یا 2026-10-07).\n"
        "اگه همون تاریخ رو دوباره بفرستی، لغوش برداشته می‌شه."
        f"{existing_note}",
        reply_markup=back_btn,
    )
...
async def send_fixed_event_reminder(context: ContextTypes.DEFAULT_TYPE, user_id: int, event: dict) -> None:
    try:
        await context.bot.send_message(
            chat_id=user_id,
...
async def check_reminders(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    همه‌ی محاسباتِ زمانی (ساعتِ الان، تاریخِ امروز، روزِ هفته) عمداً با now_tehran()
    انجام می‌شه، نه datetime.now() خام؛ چون سرورِ Railway به‌وقتِ UTC اجرا می‌شه و
    استفاده از ساعتِ سیستم باعثِ می‌شد یادآوری‌ها با اختلافِ ثابتِ فاصله‌ی UTC/ایران
    (فعلاً ۳:۳۰) با تأخیر ارسال بشن."""
    now = now_tehran()
    today_fa = PY_WEEKDAY_TO_FA[now.weekday()]
    ...
    # --- رویدادهای ثابتِ هفتگی (فاز ۱۶، مثلِ «دوشنبه فرهنگی») ---
    # هر رویداد lead_minutes ِ خودشو داره (پیش‌فرض ۱۵)، برخلافِ کلاسِ رسمی که
    # REMINDER_LEAD_MINUTES ثابتِ سراسری داره. این‌جا خودمون بر اساسِ
    # scope/برنامه فیلتر می‌کنیم (نه get_fixed_events_for_day که برای یه
    # کاربر/برنامه‌ی مشخص طراحی شده).
    ...
    if todays_events:
        for uid_str, info in users.items():
            ...
            for e_idx, e in enumerate(todays_events):
                ...
                reminder_id = f"fixedevent:{uid_str}:{today_fa}:{e_idx}"
                if now_str == reminder_time and reminder_id not in log["sent"]:
                    await send_fixed_event_reminder(context, int(uid_str), e)
                    log["sent"].append(reminder_id)
...
def _fixedevents_text_markup(user_id: int):
    ...
    for i, e in enumerate(events):
        ...
        if can_delete:
            buttons.append([InlineKeyboardButton(f"🗑 حذفِ «{e['name']}»", callback_data=f"fixedevent_del:{i}")])
...
async def fixedevent_scope_pick(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await query.edit_message_text(
        "فرمت: نام | روز | ساعتِ‌شروع-ساعتِ‌پایان | دقیقه‌ی یادآوریِ‌قبلش(اختیاری، پیش‌فرض ۱۵)\n"
        "مثال: دوشنبه فرهنگی | دوشنبه | 10:00-12:00 | 60"
    )
