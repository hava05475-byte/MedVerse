...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    text = (
        ...
        "📅 برنامه رسمی کلاس‌ها و گروه‌های عملی از بخش 📅 برنامه‌ریزی در منوی بات قابل دسترسی است.\n"
    )
    if is_admin(update.effective_user.id):
        text += (
            ...
            "\nمدیریت گروه‌بندی دروس:\n"
            "از مسیر ویرایش درس → گروه‌بندی می‌توانید برای درس‌های عملی گروه ایجاد کنید، "
            "روز و ساعت هر گروه را ثبت کنید و انتخاب گروه دانشجویان را مدیریت کنید.\n"
        )
...
async def receive_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if pending_fe:
        ...
        events = load_fixed_events()
        events.append(
            {"id": uuid.uuid4().hex[:12], "name": name, "day": day, "start": start, "end": end, "lead_minutes": lead_minutes, "scope": scope}
        )
        save_fixed_events(events)
        dbmod.log_action(
...
async def show_class_day(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not classes and not missing_groups:
        ...
        for c in classes:
            loc = f" — {c['location']}" if c.get("location") else ""
            pending = " — ⚠️ گروه هنوز مشخص نشده" if c.get("group_pending") else ""
            lines.append(f"⏰ {c['start']}–{c['end']} | {c['course']}{loc}{pending}")
        if missing_groups:
            lines.append("")
