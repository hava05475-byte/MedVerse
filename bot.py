...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Telegram Deep Link: https://t.me/<bot>?start=file_<fid>
    # بعد از همان گیت عضویتِ عادی اجرا می‌شود تا لینک، محدودیت فعلی بات را دور نزند.
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if is_admin(update.effective_user.id):
        text += (
            ...
            "/fixedevents - ثبت/حذف رویدادهای ثابتِ هفتگی (مثلِ دوشنبه فرهنگی)\n"
...
async def badge_pick_file(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for f in files:
        ...
        if not fid:
            continue
        cb = f"bdg_file:{fid}:{cat_idx}"
        if validate_callback_data(cb, context_label="badge_pick_file/fid"):
            buttons.append([InlineKeyboardButton(display_caption(f) or f.get("caption", ""), callback_data=cb)])
...
async def badge_pick_value(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(parts) == 3:
        ...
        buttons.append([InlineKeyboardButton("🔙 بازگشت", callback_data=f"bdg_cat:{program}:{course_name}:{cat_idx_str}")])
        await query.edit_message_text("کدوم نشان؟", reply_markup=InlineKeyboardMarkup(buttons))
        return

    # سازگاری با پیام/دکمه‌های قدیمی: bdg_file:{program}:{course}:{cat}:{index}
    if len(parts) == 5:
        _, program, course_name, cat_idx_str, index_str = parts
        category = category_name_by_id(int(cat_idx_str))
        data = load_data()
        files = data.get(program, {}).get(course_name, {}).get("categories", {}).get(category or "", {}).get("files", [])
        try:
    ...
    await query.edit_message_text("درخواست نشان نامعتبره؛ دوباره از منوی نشان کیفیت وارد شو.")
...
async def badge_apply(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(parts) == 3:
        ...
        save_data(data)
        dbmod.log_action(update.effective_user.id, action, term=program, detail=detail)
        await query.edit_message_text(message)
        return

    # فرمت قدیمی را هم نگه می‌داریم تا دکمه‌های قبلی از کار نیفتند.
    if len(parts) == 6:
...
async def render_cls_day(query, program: str, day: str) -> None:
    ...
    for i, c in enumerate(classes):
        ...
        if validate_callback_data(group_data, context_label=f"cls_group/program={program}/day={day}/i={i}"):
            group_label = "👥 تعیین گروه" if c.get("group_id") is None else "👥 تغییر گروه"
            buttons.append([InlineKeyboardButton(f"{group_label} «{c['course']}»", callback_data=group_data)])
...
async def cls_add_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await query.edit_message_text(
        ...
        "اگر گروه‌ها هنوز معلوم نیستن، ستون گروه رو خالی بذار؛ کلاس‌های هم‌زمان ثبت می‌شن و با «⚠️ گروه نامشخص» "
        "علامت می‌خورن تا بعداً از همین صفحه گروهشون رو تعیین کنی.)",
        reply_markup=back_btn,
    )
...
async def cls_group_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if course_id is not None:
        buttons.append([InlineKeyboardButton("⚙️ مدیریت گروه‌های این درس", callback_data=f"crs_groups:{course_id}")])
